...
%determines order of blocks
task.cues = Shuffle(task.cues);
...
task.numOrientationBins = 4;
task.memoryOrientations = [randi([5,40],task.numUniqueTrials,1),randi([50,85],task.numUniqueTrials,1),randi([95,130],task.numUniqueTrials,1),randi([140,175],task.numUniqueTrials,1)];
...
BlockA = repmat(repelem(1:length(task.cues),length(task.distractorOrientations))',size(task.trialMatrix,1)/length(repelem(1:length(task.cues),length(task.distractorOrientations))),1);

task.trialMatrix = [task.trialMatrix,BlockA];
task.matrixBalance = [task.matrixBalance,BlockA];

task.trialCuedBlocks = zeros(task.numTrialsBlock*2,size(task.trialMatrix,2),length(task.cues));

for blockk = 1:length(task.cues)
    ...
    task.trialCuedBlocks(:,:,blockk) = currentblock_matrix; 
end

task.BlockMultiplier=[];
for cc=1:length(task.cues)
    task.BlockMultiplier = [task.BlockMultiplier,[Shuffle(repelem([1,2],1,task.numTrialsBlock/2)),Shuffle(repelem([1,2],1,task.numTrialsBlock/2))]'];
end

% stupid coding - change!!!!
plusone = task.BlockMultiplier(:,1);
plusone(plusone==2) = plusone(plusone==2)+2;
task.BlockMultiplier(:,1) =plusone; 

plusone = task.BlockMultiplier(:,2);
plusone(plusone==2) = plusone(plusone==2)+0.5;
task.BlockMultiplier(:,2) =plusone; 

task.trialCuedBlocks(:,4,1) = task.trialCuedBlocks(:,4,1).*task.BlockMultiplier(:,1);
task.trialCuedBlocks(:,4,2) = task.trialCuedBlocks(:,4,2).*task.BlockMultiplier(:,2);
task.trialCuedBlocks(:,4,3) = task.trialCuedBlocks(:,4,3).*task.BlockMultiplier(:,3);

task.trialCuedBlocks_n=[];
for dim = 1:size(task.trialCuedBlocks,3)
    task.trialCuedBlocks_n=[task.trialCuedBlocks_n; task.trialCuedBlocks(:,:,dim)];
end
...
for blockk = 1:task.numBlocks
    currentblock_matrix = task.trialCuedBlocks_n(task.trialCuedBlocks_n(:,4)==blockk,:);
    currentblock_matrix = currentblock_matrix(randperm(size(currentblock_matrix, 1)),:);
...
stim.contrast = 0.5;
stim.stimSize = 7; % Diameter in degrees
stim.sf = 1; % cpd
...
stim.phaseDistractor = repmat(stim.phases, 1,(task.numTrialsBlock)*task.numBlocks*time.DistPrezT/length(stim.phases));
stim.phaseDistractor = reshape(stim.phaseDistractor, task.numTrialsBlock,time.DistPrezT, task.numBlocks);
