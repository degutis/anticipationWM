...
luminance=0.8;
range=0.85; % used to set the nonlinearity and range of contrast
nonlin_con=2;
...
stim_num=ceil((1:num_pres)/num_runs-0.0001);
stim_num=[randperm(4),randperm(4),randperm(4),randperm(4)];

stimrange=101;
PalRGB=luminance*repmat(linspace(0,1,stimrange)',1,3);
...
tot_image=10000; % total time for flashing one image
...
res.para.num_runs=num_runs;
res.para.num_images=num_images;
res.para.stim_num=stim_num;
res.para.con_range=range;
res.para.con_nonlin=nonlin_con;
...
c=clock;
filename=['./logs/CircCon_' subid '_' sprintf('%02d',c(1)) '_' sprintf('%02d',c(2)) '_' int2str(c(3)) '_' sprintf('%02d',c(4)) '_' sprintf('%02d',c(5)) '_' sprintf('%02d',fix(c(6)))];
...
num_segments=48;
con_im=zeros(1024,768);
hh=zeros(1024,768);
disp('%%% loading images ...');
poss_vals=linspace(0,1,4);
for i=1:num_images%length(files)
    for ll=1:num_segments; 
        con_im(indic{ll})=poss_vals(i); 
    end, 
    hh(:,:)=(con_im(:,:)*range+(1-range)).^nonlin_con.*ch_board(:,:);
    hh(:)=(hh(:)+1)/2;
    cgloadarray(10+(2*i)-1,1024,768,round(white*reshape(hh,resolution,1))',PalRGB,0);
    cgloadarray(10+2*i,1024,768,white-round(white*reshape(hh,resolution,1))',PalRGB,0);
end
...
draw_landold_c(landolt_c,bgcol,white,PalRGB);
...
while time<t0+t_total-t_gray
    ...
    while time<(start_next-tot_image)
        ...
        flicker=not(flicker);
            if flicker
        cgdrawsprite(10+stim_num(img_counter)*2-1,0,0);
    else
        cgdrawsprite(10+stim_num(img_counter)*2,0,0);
            end
