...
time.cue = 0.8;
time.delay = 5;
time.delay_probe1 = time.delay-(time.ISI+time.stimulus);
time.probe = 0.5; 
time.answer = 5; 
time.feedback = 1;
time.ITI = 3.5;
time.delayOneStim = time.delay; 
...
[params, oneStim, retro] = calculateStim(params); 

for bl3 = 1:length(params.Blocks)
    for stim = 1:params.numTrials
        contrasts_full1 = params.Contrasts_matrix_stim1{bl3}{stim};

        contrasts_full1 = contrasts_full1';
        
        if params.Blocks(bl3)==1
            contrasts_full2 = params.Contrasts_matrix_stim2{bl3}{stim};            
...
for bl4 = 1:length(params.Blocks)
    ...
    if params.Blocks(bl4)==1
        params.answerFull{bl4} = retro.Block{block1}(3,:);
        block1=block1+1;
        
    elseif params.Blocks(bl4) ==2
        params.answerFull{bl4} = retro.Block{block2}(3,:);
        block2=block2+1;
    end        
...
for block = 1:length(params.Blocks)
    ...
    if params.Blocks(block) == 1
        for trialNum = 1:params.numTrials
            ...
            presentation_timings{block}.type((trialNum-1)*params.nStim(1)+1) = 'Cue';
            ...
            presentation_timings{block}.type((trialNum-1)*params.nStim(1)+3) = 'Stim1';
            ...
            presentation_timings{block}.type((trialNum-1)*params.nStim(1)+5) = 'Stim2';  
            ...
            if params.cue{block}(trialNum)==2
                expectedTime = expectedTime+time.delay; 
            elseif params.cue{block}(trialNum)==1
                expectedTime = expectedTime+time.delay_probe1; 
            end
...
sca;
save(outFile, 'data', 'params', 'Ratio','time', 'presentation_timings', 'oneStim','retro');
