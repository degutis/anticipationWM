% REMARK: This function is only to check if the palette modus works.
% It currently does only log very rudamentary data.
% Adapt logging of the palette mode should be used (see direct demo).

function circcheck_run_highc_demo_pal(subid,nrun,triggermode)
...
%% save all the data to the stuct res
...
for i=1:num_images%length(files)
    ...
    cgloadarray(10+(2*i)-1,1024,768,round(reshape(hh,resolution,1))',PalRGB,0);
    cgloadarray(10+2*i,1024,768,white+1-round(reshape(hh,resolution,1))',PalRGB,0);
end
    for ll=1:num_segments; 
...
draw_landold_c(landolt_c,bgcol-1,white_landolt,PalRGB);
...
t_end=time;
res.para.tend=t_end;
if useSerial % !!! read out of button preses only works with serial in this scirpt.
