% function circcheck_run_highc_demo_direct_ramcache(subid, run_ind, triggermode)
%
...
%     circcheck_run_highc_demo_direct_ramcache('SBJNAME',1,0)
% where 
...
%   circcheck_run_highc_demo_direct_ramcache('SBJNAME',1,0)
%   circcheck_run_highc_demo_direct_ramcache('SBJNAME',2,0)
%     etc. currently up to 
%   circcheck_run_highc_demo_direct_ramcache('SBJNAME',10,0)
% 
...
%
% This version of the script does not directly load all images directly to
% the graphics card, but into main RAM. It then copies the image to the
% graphic card directly before the image is shown for the first time. This
% prevents problems if their is not enough memory on the graphics card to
% hold all images (e.g. if many should be shown). 
%
% If you experience that the images take too long to load when shown for
% the first time within each block, consider using a computer with more 
% GPU memory and use the _direct version of the script instead.
% 
...
% try to allocate memory to save all images in RAM
% (increase MATLAB RAM if not sufficient)
preloaded_images = uint8(zeros(resolution, num_images)); % init ram for preloaded images 
% init time measurement (if required)
if show_images_during_startup
    res.image_to_GPU_timeimage_to_GPU_time_test = nan(2, num_images); % init measurement of loading time
end

for img_counter=1:num_images%length(files)
    logstring(sprintf('Loading image %i/%i', img_counter, num_images))
    for ll=1:num_segments;
        con_im(indic{rvec(ll)})=round(t_4(ll,img_counter+50)*3)+1;
    end,
    hh(:,:)=bgcol+PalContrasts(con_im).*ch_board;
    % from version that directly load all data to GPU ram
%         cgloadarray(10+(2*img_counter)-1,1024,768,repmat(round(reshape(hh,resolution,1))/256',1,3));
%         cgdrawsprite(10+(2*img_counter)-1,0,0); 
%         cgflip;
%         keyboard
        
    % have all data ready in ram (as int8 to save RAM, if this is still too
    % slow try preloading all images and INCREASE Matlabs ram
    preloaded_images(:, img_counter) = round(reshape(hh,resolution,1)); % image i
    
    if show_images_during_startup
        % show images and test time to load

        % show standard image
        img_counter = img_counter; % copy variable to have same line as in script below
        % measure loading time to GPU (should be below flicker frequency)
        tic; 
        cgloadarray(11,1024,768,repmat(double(preloaded_images(:, img_counter))/256,1,3)) % load image i
        res.image_to_GPU_time_test(1, img_counter) = toc; % measure loading time to GPU (should be below flicker frequency)
        % prepare and show
        cgdrawsprite(11,0,0); 
        cgflip;
        ...
        % measure loading time to GPU (should be below flicker frequency)
        tic
        cgloadarray(12,1024,768,repmat((white+1-double(preloaded_images(:, img_counter)))/256,1,3)) % load image i flipped
        res.image_to_GPU_time_test(2, img_counter) = toc;
        % prepare and show
        cgdrawsprite(12,0,0); 
        cgflip;
        
        % log timing
        logstring(sprintf('  Test time RAM to GPU: standard %f, flipped %f', res.image_to_GPU_time_test(:, img_counter)))
    end
end
for ll=1:num_segments;
...
cgloadarray(1,1024,768,repmat(round(reshape(hh,resolution,1))/256',1,3)); % standard background
cgloadarray(2,1024,768,repmat((white+1-round(reshape(hh,resolution,1)))/256',1,3)); % flipped standard background in 1
...
res.image_to_GPU_time = nan(2, num_images); % init measurement of loading time
% last_img_counter = 0; % init
t_start_images=time;
show_log_image_change = false;
while time<t0+t_total-t_gray
    img_counter=img_counter+1;
    logstring(sprintf('Preparing image number : %03.0f',img_counter));
    load_new_normal = true;
    load_new_flipped = true;
    ...
    while time<start_next
        ...
        flicker=not(flicker);
        if flicker
            background_sprt = 3; % background_sprt = 10+img_counter*2-1; % in direct version
            if load_new_normal
                % image changed, load new from memory to GPU
                tic
                    cgloadarray(background_sprt,1024,768,repmat(double(preloaded_images(:, img_counter))/256,1,3)) % load image i
                res.image_to_GPU_time(1, img_counter) = toc;
                load_new_normal = false;
            end
        else
            background_sprt = 4; % background_sprt = 10+img_counter*2; % in direct version
            if load_new_flipped
                % image changed, load new from memory to GPU
                tic
                    cgloadarray(background_sprt,1024,768,repmat((white+1-double(preloaded_images(:, img_counter)))/256,1,3)) % load image i flipped
                res.image_to_GPU_time(2, img_counter) = toc;
                load_new_flipped = false;
                show_log_image_change = true; % only show log after flipped has been loaded
            end
        end
        cgdrawsprite(background_sprt,0,0);        
        ...
        if last_landolt ~= landolt 
            res = loginput(res, useSerial); % read input from last run
            logstring(['Landolt C: ' int2str(landolt) ': ' landolt_open(landolt)]); % log new landolt
            last_landolt = landolt;
        end
        
        if show_log_image_change
            logstring(['Image: ' int2str(img_counter) sprintf('. Loading time standard %f, flipped %f', res.image_to_GPU_time(:, img_counter))]); % log new run
            show_log_image_change = false;
...
% show all loading times
% res.image_to_GPU_time
