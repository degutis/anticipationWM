function circcheck_pal_run_demo(subid,nrun,triggermode)
...
config_display( 2, 3, [0 0 0], [luminance luminance luminance], 'Arial', 24, 5 ,8)
%config_keyboard;
...
for i=1:num_images%length(files)
    for ll=1:num_segments; 
        con_im(indic{rvec(ll)})=t_4(ll,i+50); 
    end, 
...
hh(:,:)=(con_im(:,:)*range+(1-range)).^nonlin_con.*ch_board(:,:);
hh(:)=(hh(:)+1)/2;
cgloadarray(1,1024,768,round(white*reshape(hh,resolution,1))',PalRGB,0);
cgloadarray(2,1024,768,white-round(white*reshape(hh,resolution,1))',PalRGB,0);
...
draw_landold_c(landolt_c,bgcol-1,white,PalRGB);
...
while time<t0+t_total-t_gray
    ...
    while time<start_next
        ...
        flicker=not(flicker);
            if flicker
        cgdrawsprite(10+img_counter*2-1,0,0);
    else
        cgdrawsprite(10+img_counter*2,0,0);
            end
